...
const Settings = ({
  ...
  console.log('selectedOpions', selectedOptions)
              ...
              <div className={`settings-options settings-options-${setting.type}`}>
                {
                  setting.options.map(option => (
                    <Option
                      ...
                      active={setting.multiSelect && Array.isArray(config?.[setting.prop]) ? config?.[setting.prop].includes(option.value) : config?.[setting.prop] === option.value}
                      onSelectOption={(value) => {
                        setting.multiSelect ? onSelectMultiOption(setting.prop, value) : onSelectOption(setting.prop, value)
                      }
                      }
                    />
                  ))
